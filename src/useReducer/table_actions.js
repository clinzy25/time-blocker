export const SET_BLOCK_INTERVAL = 'SET_BLOCK_INTERVAL';
export const SET_BLOCK_SIZE = 'SET_BLOCK_SIZE';
export const GET_TIMES = 'GET_TIMES';
export const SHIFT_DAYS = 'SHIFT_DAYS';
export const SET_TIME_RANGE = 'SET_TIME_RANGE';
export const ADD_TASK = 'ADD_TASK';
export const CLEAR_TABLE = 'CLEAR_TABLE';
export const DELETE_TASK = 'DELETE_TASK';
export const SET_TABLE_TITLE = 'SET_TABLE_TITLE';
export const SET_CURRENT_TIME_ON_TOP = 'SET_CURRENT_TIME_ON_TOP';
export const SET_TASK_TEXT = 'SET_TASK_TEXT';
export const SET_USER = 'SET_USER';
export const GET_TABLE_SETTINGS_BEGIN = 'GET_TABLE_SETTINGS_BEGIN';
export const GET_TABLE_SETTINGS_SUCCESS = 'GET_TABLE_SETTINGS_SUCCESS';
export const SET_USER_BEGIN = 'GET_TABLE_SETTINGS_BEGIN';
export const SET_USER_SUCCESS = 'SET_USER_SUCCESS';
